...
initUI :: proc(ui: ^UI) {
	...
	if err := SDL.UpdateTexture(atlas, nil, raw_data(pixels), 4 * mu.DEFAULT_ATLAS_WIDTH);
	   err != 0 {
...
generateCorridorRect :: proc(room: ^SDL.Rect, direction: Direction) -> SDL.Rect {
	switch direction {
	case .North:
		return SDL.Rect{room.x, room.y - room.h, room.w, room.h}
	case .East:
		return SDL.Rect{room.x + room.w, room.y, room.w, room.h}
	case .South:
		return SDL.Rect{room.x, room.y + room.h, room.w, room.h}
	case .West:
		return SDL.Rect{room.x - room.w, room.y, room.w, room.h}
...
handleMenuWindow :: proc(ui: ^UI, logic: ^Logic) {
	...
	if mu.begin_window(
		ui.ctx,
		"Menu",
		mu.Rect{0, 0, ui.slider.value, h},
		mu.Options{.NO_CLOSE, .NO_RESIZE, .ALIGN_CENTER},
	) {
		r := mu.get_current_container(ui.ctx).body

		if .LEFT in ui.ctx.mouse_pressed_bits {
			if ui.ctx.mouse_pos.x >= ui.slider.value - 12 &&
			   ui.ctx.mouse_pos.x <= ui.slider.value + 12 {
		...
		if ui.slider.captured {
			ui.slider.value = clamp(
				ui.ctx.mouse_pos.x,
				DEFAULT_SLIDER_VALUE,
				w - DEFAULT_SLIDER_VALUE,
			)
		}
		...
		if .ACTIVE in mu.header(ui.ctx, "Generation", mu.Options{.EXPANDED}) {
			...
			if .SUBMIT in mu.button(ui.ctx, "Generate") {
				...
				height := readBufferValue(
					ui.menu.size.height.buffer[:],
					ui.menu.size.height.length,
				)
		...
		if .ACTIVE in mu.header(ui.ctx, "Solution") {
			...
			if .ACTIVE in mu.begin_treenode(ui.ctx, "Coords", mu.Options{.CLOSED}) {
				if .ACTIVE in mu.begin_treenode(ui.ctx, "Start", mu.Options{.CLOSED}) {
					mu.textbox(
						ui.ctx,
						ui.menu.solution.start.x.buffer[:],
						&ui.menu.solution.start.x.length,
					)
					mu.textbox(
						ui.ctx,
						ui.menu.solution.start.y.buffer[:],
						&ui.menu.solution.start.y.length,
					)
				...
				if .ACTIVE in mu.begin_treenode(ui.ctx, "Finish", mu.Options{.CLOSED}) {
					mu.textbox(
						ui.ctx,
						ui.menu.solution.finish.x.buffer[:],
						&ui.menu.solution.finish.x.length,
					)
					mu.textbox(
						ui.ctx,
						ui.menu.solution.finish.y.buffer[:],
						&ui.menu.solution.finish.y.length,
					)
					mu.end_treenode(ui.ctx)
				}
				mu.end_treenode(ui.ctx)
			...
			if .ACTIVE in mu.begin_treenode(ui.ctx, "Method", mu.Options{.CLOSED}) {
				for method in METHOD {
					if .SUBMIT in mu.button(ui.ctx, MethodNames[method]) {
						logic.method = method
					}
				}
				mu.end_treenode(ui.ctx)
			...
			solve := strings.concatenate({"Solve ( ", MethodNames[logic.method], " )"})
			defer delete(solve)
			if .SUBMIT in mu.button(ui.ctx, solve) {
				...
				logic.maxstep = findPath(
					&logic.path,
					-1,
					&logic.labyrinth,
					&logic.coords.start,
					&logic.coords.finish,
					MethodProcs[logic.method],
				)
...
handleFieldWindow :: proc(ui: ^UI, logic: ^Logic) {
	...
	if mu.begin_window(
		ui.ctx,
		"Field",
		mu.Rect{ui.slider.value, 0, w - ui.slider.value, h},
		mu.Options{.NO_CLOSE, .NO_RESIZE, .NO_FRAME, .ALIGN_CENTER},
	) {
		container := mu.get_current_container(ui.ctx).body

		ui.field.origin = {container.x, container.y}
		ui.field.size = {container.w, container.h}
...
adjustFieldScale :: proc(ui: ^UI, labyrinth: ^Labyrinth) {
	...
	field.scale =
		f32(min(field.size.x, field.size.y)) /
		(max(f32(labyrinth.size.x), f32(labyrinth.size.y)) * 2)
	field.translation.x = i32(
		math.ceil(f32(field.size.x / 2) - (f32(labyrinth.size.x) - f32(0.5)) * field.scale),
	)
	field.translation.y = i32(
		math.ceil(f32(field.size.y / 2) - (f32(labyrinth.size.y) - f32(0.5)) * field.scale),
	)
}
